...
var battleships = {
	...
	stopUserByStreamId: function(streamId) {
		database.findUser({streamId: streamId}, function(err, user) {
			if(err || !user) {
				return console.log('no user found - presume premature disconnect');
			}
	...
	initUser: function (userObj, peer) {
		// verify gameBoard
		var totalMarkers = JSON.stringify(userObj.board).match(/1/g).length;
		if(totalMarkers != totalHitsInGame) {
			return peer.emit('err', 'NotEnoughMarkers', 'Please add the correct number of ships: ' + totalHitsInGame);
		}
